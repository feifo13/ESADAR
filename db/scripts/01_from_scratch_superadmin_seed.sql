...
CREATE TABLE user_auth_identities (
  ...
  CONSTRAINT fk_user_auth_identities_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE company_collecting_settings (
  ...
  is_mercado_pago_enabled TINYINT(1) NOT NULL DEFAULT 0,
...
CREATE TABLE customer_addresses (
  ...
  dwelling_type ENUM('HOUSE','APARTMENT') NULL,
...
CREATE TABLE orders (
  ...
  customer_first_name_snapshot VARCHAR(100) NULL,
  customer_last_name_snapshot VARCHAR(100) NULL,
  customer_email_snapshot VARCHAR(255) NULL,
  customer_phone_snapshot VARCHAR(50) NULL,
  customer_address_line_snapshot VARCHAR(255) NULL,
  customer_city_snapshot VARCHAR(120) NULL,
  customer_state_snapshot VARCHAR(120) NULL,
  customer_country_snapshot VARCHAR(120) NULL,
  customer_postal_code_snapshot VARCHAR(30) NULL,
  customer_dwelling_type_snapshot ENUM('HOUSE','APARTMENT') NULL,
  customer_apartment_snapshot VARCHAR(80) NULL,
  customer_delivery_notes_snapshot TEXT NULL,
...
CREATE TABLE mercado_pago_checkout_preferences (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  order_id BIGINT UNSIGNED NOT NULL,
  order_number VARCHAR(50) NOT NULL,
  environment ENUM('test','production') NOT NULL DEFAULT 'test',
