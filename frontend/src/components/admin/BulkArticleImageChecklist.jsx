...
export const IMAGE_ROLE_DEFINITIONS = [
  ...
  {
    key: "condition",
    label: "Detalle de estado",
    helper: "Subelo si la prenda tiene detalle",
  },
...
export function createEmptyImageState() {
  return IMAGE_ROLE_DEFINITIONS.reduce(
    (accumulator, role) => ({
      ...accumulator,
      [role.key]: {
        file: null,
...
export default function BulkArticleImageChecklist({
  value,
  title,
  onChange,
  hasConditionWarning,
      ...
      {IMAGE_ROLE_DEFINITIONS.map((role) => {
        ...
        return (
          <article
            key={role.key}
            className={
              hasFile ? "bulk-image-card is-complete" : "bulk-image-card"
            }
          >
            ...
            {current?.preview ? (
              <SmartImage
                ...
                alt={
                  current.altText ||
                  `${title || "Prenda"} - ${role.label.toLowerCase()}`
                }
            ...
            <label className="button button-secondary button-compact">
              ...
              <input
                ...
                hidden
                onChange={(event) =>
                  replaceFile(role.key, event.target.files?.[0] || null)
                }
              />
            </label>
            ...
            <div className="inline-action-group">
              ...
              {hasFile ? (
                <button
                  ...
                  onClick={() => replaceFile(role.key, null)}
                >
                  Borrar
                </button>
              ) : null}
