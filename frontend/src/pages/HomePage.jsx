    ...
    <div className="container page-stack">
      <section className="hero-panel">
        <div>
          ...
          <p className="hero-copy">
            Sin modelos. Sin ruido. Solo prendas elegidas una a una para que la ropa hable por sí misma.
          </p>
        </div>
      ...
      <section className="catalog-shell">
        <div className="catalog-toolbar sticky-panel">
          <ArticleFilters value={filters} onChange={applyFilters} onReset={() => applyFilters(initialFilters)} />
          <div className="view-toggle">
            <button type="button" className={view === 'grid' ? 'active' : ''} onClick={() => setView('grid')}>Grilla</button>
        ...
        {loading ? (
          ...
          <>
            ...
            {!items.length ? <div className="section-card centered-card">No encontramos artículos con esos filtros.</div> : null}

            {canLoadMore ? (
              <div className="load-more-row">
                <button type="button" className="button button-primary" onClick={() => setPage((current) => current + 1)} disabled={loadingMore}>
