...
const formatSource = (source) =>
  SOURCE_LABELS[source] || source || "Sin origen";
      ...
      try {
        ...
        if (!ignore)
          notifyMobileStatus({
            type: "error",
            icon: "error",
            message: err.message || "No se pudo cargar el lead.",
          });
      } finally {
  ...
  async function handleStatusSubmit(event) {
    ...
    try {
      ...
      notifyMobileStatus({
        type: "error",
        icon: "error",
        message: err.message || "No se pudo actualizar el lead.",
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="container page-stack admin-page-shell admin-detail-page">
      ...
      <section className="section-card page-stack">
        ...
        {!loading && !lead ? (
          <p className="muted-copy">Lead no encontrado.</p>
        ) : null}
        {lead ? (
          <>
            ...
            <div className="admin-detail-meta admin-detail-meta--grid">
              ...
              <p className="summary-line">
                <span>WhatsApp</span>
                <strong>{lead.phone || "Sin telefono"}</strong>
              </p>
              <p className="summary-line">
                <span>Instagram</span>
                <strong>{lead.instagram || "Sin Instagram"}</strong>
              </p>
              <p className="summary-line">
                <span>Alta</span>
            ...
            <div className="page-stack-sm">
              ...
              <div className="preference-chip-list">
                {formatPreferences(lead.preferences).length ? (
                  formatPreferences(lead.preferences).map((item) => (
                    <span
                      key={`${lead.id}-${item}`}
            ...
            <div className="page-stack-sm">
              ...
              {(lead.wishlists || []).length ? (
                <div className="history-list">
                  {lead.wishlists.map((wishlist) => (
                    <article key={wishlist.id} className="history-row">
