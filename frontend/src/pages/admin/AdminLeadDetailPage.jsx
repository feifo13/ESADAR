      ...
      <section className="section-card page-stack">
        ...
        {lead ? (<>
          ...
          <div className="admin-detail-meta admin-detail-meta--grid">
            <p className="summary-line"><span>Origen</span><strong>{formatSource(lead.source)}</strong></p><p className="summary-line"><span>Email</span><strong>{lead.email || 'Sin email'}</strong></p><p className="summary-line"><span>WhatsApp</span><strong>{lead.phone || 'Sin teléfono'}</strong></p><p className="summary-line"><span>Instagram</span><strong>{lead.instagram || 'Sin Instagram'}</strong></p><p className="summary-line"><span>Alta</span><strong>{formatDate(lead.createdAt)}</strong></p><p className="summary-line"><span>Actualización</span><strong>{formatDate(lead.updatedAt)}</strong></p>
          </div>
          ...
          <div className="page-stack-sm"><p className="section-kicker">Alertas</p>{(lead.alerts || []).length ? <div className="history-list">{lead.alerts.map((alert) => <article key={alert.id} className="history-row"><div><strong>{alert.alertType}</strong><p className="muted-copy">{alert.articleTitle || 'Sin artículo asociado'} · {formatDate(alert.createdAt)}</p></div><div className="table-actions"><StatusBadge status={alert.status} labels={ALERT_STATUS_LABELS} />{alert.articleSlug ? <Link className="ghost-button" to={articlePath({ slug: alert.articleSlug, articleId: alert.articleId })}>Ver artículo</Link> : null}</div></article>)}</div> : <p className="muted-copy">Sin alertas asociadas.</p>}</div>
